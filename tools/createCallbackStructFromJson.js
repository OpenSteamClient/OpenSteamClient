...
while (!found) {
    found = fs.existsSync(projectDir + "/OpenSteamClient.sln");
    if (!found) {
...
structs.forEach((struct) => {
    var file = fs.createWriteStream(projectDir+"/OpenSteamworks/Callbacks/Structs/"+struct.struct+".cs", {
        flags: 'w'
    })

    var writeLine = (line) => file.write(`${line}\n`);
    
    writeLine("using System;");
    writeLine("using System.Runtime.InteropServices;");
    writeLine("using OpenSteamworks.Enums;");
    writeLine("using OpenSteamworks.Structs;");
    writeLine("");
    writeLine("namespace OpenSteamworks.Callbacks.Structs;");
    writeLine("");
    writeLine("[StructLayout(LayoutKind.Sequential)]");
    writeLine("public unsafe struct " + struct.struct);
    writeLine("{");
    struct.fields.forEach((field) => {
        if (field.fieldtype == "const char *") {
            field.fieldtype = "string";
        } else if (field.fieldtype == "uint32") {
            field.fieldtype = "UInt32";
        } else if (field.fieldtype == "uint64") {
            field.fieldtype = "UInt64";
        }
        writeLine(`	public ${field.fieldtype} ${field. fieldname};`);
    })
    writeLine("};");
});