...
public:
    ...
    virtual bool SendP2PPacket( CSteamID steamIDRemote, const void *pubData, uint32 cubData, EP2PSend eP2PSendType, int32 iVirtualPort ) = 0; //argc: 6, index 1
    
    // returns true if any data is available for read, and the amount of data that will need to be read
    virtual bool IsP2PPacketAvailable( uint32 *pcubMsgSize, int32 iVirtualPort ) = 0; //argc: 2, index 2
    
    // reads in a packet that has been sent from another user via SendP2PPacket()
    // returns the size of the message and the steamID of the user who sent it in the last two parameters
    // if the buffer passed in is too small, the message will be truncated
    // this call is not blocking, and will return false if no data is available
    virtual bool ReadP2PPacket( void *pubDest, uint32 cubDest, uint32 *pcubMsgSize, CSteamID *psteamIDRemote, int32 iVirtualPort ) = 0; //argc: 5, index 3
    ...
    // WARNING: Argument count doesn't match argc! Remove this once this has been corrected!
    virtual bool CloseP2PChannelWithUser( CSteamID steamIDRemote, int32 iVirtualPort ) = 0; //argc: 3, index 6
    ...
    // WARNING: Argument count doesn't match argc! Remove this once this has been corrected!
    virtual SNetListenSocket_t CreateListenSocket( int32 nVirtualP2PPort, uint32 nIP, uint16 nPort, bool bAllowUseOfPacketRelay ) = 0; //argc: 8, index 9
    ...
    // WARNING: Argument count doesn't match argc! Remove this once this has been corrected!
    virtual SNetSocket_t CreateP2PConnectionSocket( CSteamID steamIDTarget, int32 nVirtualPort, int32 nTimeoutSec, bool bAllowUseOfPacketRelay ) = 0; //argc: 5, index 10
    // WARNING: Argument count doesn't match argc! Remove this once this has been corrected!
    virtual SNetSocket_t CreateConnectionSocket( uint32 nIP, uint16 nPort, int32 nTimeoutSec ) = 0; //argc: 7, index 11
    ...
    virtual bool GetSocketInfo( SNetSocket_t hSocket, CSteamID *pSteamIDRemote, int32 *peSocketStatus, uint32 *punIPRemote, uint16 *punPortRemote ) = 0; //argc: 5, index 19
    
    // returns which local port the listen socket is bound to
    // *pnIP and *pnPort will be 0 if the socket is set to listen for P2P connections only
    virtual bool GetListenSocketInfo( SNetListenSocket_t hListenSocket, uint32 *pnIP, uint16 *pnPort ) = 0; //argc: 3, index 20
    
    // returns true to describe how the socket ended up connecting
    virtual ESNetSocketConnectionType GetSocketConnectionType( SNetSocket_t hSocket ) = 0; //argc: 1, index 21
    
    // max packet size, in bytes
    virtual int32 GetMaxPacketSize( SNetSocket_t hSocket ) = 0; //argc: 1, index 22
};
